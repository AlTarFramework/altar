...
newSource(PyObject *, PyObject * args)
{
    ...
    int status = PyArg_ParseTuple(args, "d:newSource", &g, &Gsm, &nu, &mu, &drho);
    ...
    // make a new source
    source_t * source = new source_t(g, Gsm, nu, mu, drho);
...
layout(PyObject *, PyObject * args)
{
    // storage
    PyObject * pySource;
    std::size_t x0Idx, asIdx, hsIdx, qIdx, offsetIdx;
    ...
    // attach the map
    source->layout(x0Idx, asIdx, hsIdx, qIdx, offsetIdx);
