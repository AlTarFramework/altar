...
class AdaptiveMetropolis(altar.component, family="altar.samplers.adaptivemetropolis", implements=sampler):
    """
    The adaptive Metropolis algorithm as a sampler of the posterior distribution
    It calculates the correlation between initial samples and current samples. 
    If the correlation is below certain value (decorrelated), we stop the Markov Chain iteration.  
    """
    ...
    steps.doc = 'the max length of each Markov chain'

    correlationSteps = altar.properties.int(default=20)
    correlationSteps.doc = 'the length of each Markov chain to calculate the correlations' 

    correlationThreshold = altar.properties.float(default=0.4)
    correlationThreshold.dpc = 'the threshold of correlations to stop the iteration'
    ...
    def walkChains(self, model, step):
        ...
        # make a copy of the original θ
        initθ = altar.matrix(shape=θ.shape)
        initθ.copy(θ)
        
        # correlation check iterations
        iterMaxSteps = self.steps/self.correlationSteps    
        iterStep = 0  
        correlation = 1.0 
        correlationThreshold = self.correlationThreshold
    
        # step all chains together
        while iterStep < iterMaxSteps and correlation > correlationThreshold: 
            # each correlation check step
            for step in range(self.correlationSteps):
            ...
            # end of MC updates for each correlation step
            # calculate the correlation between current samples and initial samples        
            correlation = self.calc_correlation(θ, initθ, samples, parameters)
            print("debug", correlation)
            iterStep += 1
        # all done
        return accepted, rejected, unlikely

    def calc_correlation(self, θ, initθ, samples, parameters):
        """ 
        calculate correlations between two sets of samples
        """
        correlation = 0.0
        ...
        for sample in range(samples):
            ...
            theta = θ.getRow(sample)
            init_theta = initθ.getRow(sample)
            corrSample = altar.stats.correlation(theta, init_theta)    
            # take the max value
            correlation = max(correlation, corrSample)
        return correlation 
